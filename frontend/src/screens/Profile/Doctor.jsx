...
export const DoctorProfile = () => {
    ...
    useEffect(() => {
        doctorInfo && Object.keys(doctorInfo).map(item => {
            if (item === "dateOfBirth") {
...
console.log("check",doctorInfo)
    ...
    >
        ...
        >
            <Box
                sx={{ background: "linear-gradient(270deg, #FFF 0%, #5642AA 100%);" }}
                width="100%"
                display="flex"
                height="11rem"
                alignItems="center"
            ...
            <Box padding="0 4rem" display="flex">
                {doctorDetail.gender=='Male'?(<img src={maleIcon} style={{ marginTop: "-2rem" }} />):(<img src={femaleIcon} style={{ marginTop: "-2rem" }} />)}
                <Box
                    marginLeft="auto"
                    padding="2rem"
                    paddingRight={0}
            ...
            <Box display="flex" alignItems="center" margin="2rem 0">
                <hr style={{ height: "2px", background: `${theme["blue-150"]}`, width: "5%" }} />
                <Typography fontWeight="600" fontSize="1.8rem" padding="0 0.5rem" width="max-content" fontFamily='Poppins' color={`${theme.doctor.primary}`}>Personal Information</Typography>
            ...
            <Box display="flex" gap="4rem" flexWrap="wrap">
                {doctorDetails.personal.map(({ name, id, type,disabled }) => <Box
                    ...
                    key={id}
                >
                    <Typography minWidth="max-content" fontWeight="bold" paddingRight="1rem" marginRight="1rem" borderRight="1px solid lightgray" fontFamily='Poppins'>
                        {name}
                    </Typography>
                    <TextField
            ...
            <Box display="flex" alignItems="center" margin="2rem 0">
                ...
                <Typography fontFamily='Poppins' fontWeight="600" fontSize="1.8rem" padding="0 0.5rem" width="max-content" color={`${theme.doctor.primary}`}>Contact Information</Typography>
                <hr style={{ height: "2px", background: `${theme.doctor.background}`, width: "calc(100% - 25rem)" }} />
            </Box>
            <Box display="flex" gap="4rem" flexWrap="wrap">
                {doctorDetails.contact.map(({ name, id, type, disabled }) =>
                    <Box
                        border="1px solid lightgray"
                        borderRadius="7px"
                        padding="0.3rem 1rem"
                        ...
                        key={id}
                    >
                        <Typography minWidth="max-content" fontFamily='Poppins'fontWeight="bold" paddingRight="1rem" marginRight="1rem" borderRight="1px solid lightgray">
                            {name}
                        </Typography>
                        <TextField
                            variant="standard"
            ...
            <Box display="flex" alignItems="center" margin="2rem 0">
                <hr style={{ height: "2px", background: `${theme.doctor.primary}`, width: "5%" }} />
                <Typography fontFamily='Poppins' fontWeight="600" fontSize="1.8rem" padding="0 0.5rem" width="max-content" color={`${theme.doctor.primary}`}>Work Information</Typography>
                <hr style={{ height: "2px", background: `${theme.doctor.primary}`, width: "calc(100% - 25rem)" }} />
            </Box>
            <Box display="flex" gap="4rem" flexWrap="wrap">
                {doctorDetails.work.map(({ name, id, type, disabled }) =>
                    <Box
                        border="1px solid lightgray"
                        borderRadius="7px"
                        padding="0.3rem 1rem"
